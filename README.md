...
| ISC | `ISC_all` | `dry-isc`, `isc` | pairwise and leave-one-out ISC (vertex-level and per-parcellation). | `results/{dataset}/pwISC/` |
...
| functional connectivity | `fc_all` | `dry-fc`, `fc` | pearson correlation FC matrices per parcellation atlas, per confound set, resampled and non-resampled. | `results/{dataset}/FC/{atlas}/` |
| archiving | `archive_results` | `dry-archive`, `archive` | packages `results/{dataset}` into a SquashFS image at `{archive_dir}/{archive_file}` (both templated on `{dataset}`, project space). Re-running after adding subjects appends to the existing image rather than overwriting it (no `-noappend`). | `archive_dir`/`archive_file` (project space) |
| retrieval | `retrieve_archive` | `dry-retrieve`, `retrieve` | extracts the archived SquashFS image back into `results/{dataset}` (`unsquashfs -f`, so it merges into/overwrites anything already there). Not wired into the main DAG on purpose — it won't trigger `archive_results` if the archive doesn't exist yet. | `results/{dataset}` |
...
> **Offloading subjects from scratch:** the subject list for every stage is discovered from what's physically present in `results/{dataset}/fmriprep` (not from `participants.tsv`) — deleting a subject's `fmriprep/sub-X` folder drops them out of every later stage's target list too. When freeing scratch space after archiving, delete a subject's data from **every** `results/{dataset}/*` subfolder, including `fmriprep/sub-X`. If you leave `fmriprep/sub-X` in place but remove a downstream folder (e.g. `denoised/sub-X`), the subject is still discovered and Snakemake will try to regenerate the now-missing downstream output instead of leaving them archived.
